...
# define which session to use
SESSION <- "ID07_01"
CHANNEL <- "smo2_dom"
RATER <- "Rater_A"

# what each element means, drawn from the university palette in its own order:
# the measured signal recedes, the fit is the primary series, and the
# breakpoint -- the one thing the panel is about -- is the gold flag.
COL_DATA <- FI_INK
COL_FIT <- FI_BLUE
COL_BP <- FI_GOLD
COL_DERIV <- FI_CORAL
...
# ---- run every method ------------------------------------------------------
# Listed in panel order. With ncol = 2 each row pairs two methods that share a
# signal, so the reader compares methods rather than scales.
#   A B  SmO2 vs time       visual (rated by eye) | raw (segmented)
#   C D  stage-mean SmO2    mean (segmented)      | poly (4th order)
#   E F  power/SmO2 ratio   ratio (segmented)     | dmax_ratio
#   G H  what is left       dmax (deoxygenation)  | slope (%/min)
bps <- list(
...
# titles/y_labs above stay plain text to write; the markdown tag that turns
# "2" into a real subscript (rendered by ggtext::element_markdown() in
# theme_poster()) is added here instead of cluttering every cell
...
plot_method <- function(key, tag) {
  ...
  if (is.null(ylim)) {
    p <- p +
      scale_y_continuous(expand = expansion(mult = c(0.05, 0.06 + 0.12 * rows)))
...
# Sized from the per-panel box so each of the 8 sub-plots is exactly
# FI_FIG_W x FI_FIG_H on the A0 (84.1 x 118.9 cm) sheet: 42 x 56 cm, i.e. half
# the poster width. Rendered once at exactly that size, so the theme's point
# sizes are the printed point sizes -- never scale the PNG afterwards.
...
icc_all <- readRDS(file.path("Data", "AnalysisData", "icc_all.rds"))
